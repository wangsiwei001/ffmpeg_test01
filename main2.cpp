...
int main() {
    ...
    if (avformat_open_input(&fmt_ctx, src.data(), NULL, NULL) < 0) {
    ...
    av_dump_format(fmt_ctx, 0, src.data(), 0);
    ...
    if (ret < 0) {
        fprintf(stderr, "Could not find %s stream in input file '%s'\n",
                av_get_media_type_string(AVMEDIA_TYPE_VIDEO), src.data());
        return ret;
    }
    ...
    */
    ...
    while (av_read_frame(fmt_ctx, &avpkt) >= 0) {
        ...
        */
        ...
        if (avpkt.stream_index == stream_index) {
            if (decode_write_frame(dst.data(), c, img_convert_ctx, frame, &frame_count, &avpkt, 0) < 0)
                exit(1);
        }
    ...
    avpkt.data = NULL;
    avpkt.size = 0;
    decode_write_frame(dst.data(), c, img_convert_ctx, frame, &frame_count, &avpkt, 1);
